...
def ytest(device, net, test_loader, mode="01"):
    ...
    for inputs, labels in test_loader:
        ...
        outputs,enhanced_tokens = net(inputs)
...
if __name__ == '__main__':
    ...
    number = len(y_test) // testSizeNumber
    features = []
    labels = y_pred_test+ 1  # Shift label range to [1, Classes]
    ...
    for i in range(number):
        ...
        enhanced_tokens,temp2 = net(temp)
        fea = temp2.cpu().detach().numpy()

        for j, label in enumerate(labels[i * testSizeNumber:(i + 1) * testSizeNumber]):
            class_features[label].append(fea[j])

        del temp, temp2

    if (i + 1) * testSizeNumber < len(y_test):
        temp = Xtest[(i + 1) * testSizeNumber:len(y_test)]
        temp = torch.tensor(temp, dtype=torch.float).cuda()
        outs,temp2 = net(temp)
        fea = temp2.cpu().detach().numpy()

        for j, label in enumerate(labels[(i + 1) * testSizeNumber:]):
            class_features[label].append(fea[j])
    ...
    for label, feats in class_features.items():
        sample_count = min(max_samples_per_class, len(feats))
        selected_features.extend(feats[:sample_count])
        selected_labels.extend([label] * sample_count)
    ...
    color_indices = np.arange(1, args.cls + 1)
    colors = list_to_colormap(color_indices)
    cmap = ListedColormap(colors)

    # Define discrete colormap with BoundaryNorm
    norm = BoundaryNorm(np.arange(1, args.cls + 2), cmap.N)

    # 调整图像尺寸
    plt.figure(figsize=(6, 5))  # 增大图像尺寸

    # Scatter plot with custom colormap
    fig = plt.scatter(vis_x, vis_y, c=labels, cmap=cmap, norm=norm, marker='.')

    # Create a colorbar with discrete labels
    cbar = plt.colorbar(fig, ticks=np.arange(1, args.cls + 1))
    print("features ")
    # 调整colorbar的字体大小
    ...
    # Save the figure
    plt.savefig("ctTSNE_" + args.dataset_name + ".png", dpi=300, format='png', bbox_inches='tight', pad_inches=0)
    plt.savefig("ctTSNE_" + args.dataset_name + ".eps", dpi=300, format='eps', bbox_inches='tight', pad_inches=0)
    print("features Done")
    plt.clf()